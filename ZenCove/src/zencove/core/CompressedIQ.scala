...
/** 压缩队列。
  ...
  * @param slotType
  */
abstract class CompressedIQ[T <: IssueSlot](
    ...
    for (i <- 0 until decodeWidth) {
      ...
      for (j <- i until depth) {
        when(queueIO.pushPorts(i).valid && validFall(j - i)) {
          // 定位匹配，则将槽入队
          if (issConfig.alterImpl) queue(j).push(queueIO.pushPorts(i).payload)
  ...
  def genCompressLogic() = {
    // 压缩逻辑
    val popCounts = Vec(UInt(log2Up(issueWidth + 1) bits), depth)
    ...
    for (i <- 0 until depth) {
      if (!issConfig.alterImpl) queue(i) := queueNext(i)
  ...
  def genGlobalWakeup(prf: PhysRegFile, rPorts: Int) = prf.clearBusys.foreach { port =>
    for (i <- 0 until depth) {
      // 远程唤醒（监听写busy广播）
      for (j <- 0 until rPorts)
        when(port.valid && port.payload === queue(i).payload.rRegs(j).payload) {
          if (issConfig.alterImpl) queue(i).payload.rRegs(j).valid := True
  ...
  def genFlushLogic() = when(queueFlush) {
    // flush最高优先级
    for (i <- 0 until depth) {
      if (issConfig.alterImpl) queue(i).valid := False
      else queueNext(i).valid := False
    }
  }
  override def setup(pipeline: CPU): Unit = {
    val PRF = pipeline.service[PhysRegFile]
