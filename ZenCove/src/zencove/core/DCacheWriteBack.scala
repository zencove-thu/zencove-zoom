...
class DCacheWriteBack(config: ZencoveConfig) extends Plugin[MemPipeline] with DBus {
  ...
  val dBus = Axi4(config.axi).setIdle()
  ...
  override def build(pipeline: MemPipeline): Unit = pipeline plug new Area {
    ...
    pipeline.MEM2 plug new Area {
      ...
      val noExcept = !input(EXCEPTION_OCCURRED)
      ...
      val dirtyLineWritebackFSM = new StateMachine {
        ...
        val waitB = dcache.waitForB generate new State
        ...
        writeMem.whenIsActive {
          ...
          when(w.ready) {
            rspId.increment()
            when(w.last) {
              if (dcache.waitForB) goto(waitB) else goto(stateBoot)
            }
          }
        }

        dcache.waitForB generate waitB.whenIsActive {
          arbitration.haltItself.set()
          val b = dBus.b
          when(b.valid) {
            goto(stateBoot)
      ...
      val CACHEFSM = new StateMachine {
        ...
        stateBoot.whenIsActive {
          when(arbitration.isValidOnEntry && isCACHE && cacheSel === CacheSel.D) {
            switch(input(ISSUE_SLOT).uop.cacheOp) {
              ...
              is(FILL_OR_HIT_WRITEBACK_INVALIDATE) {
                arbitration.haltItself.set()
                finishAllWay := False
                wayCounter := 0
                goto(checkWay)
        ...
        // TODO: Index Invalidate Need recheck
      ...
      val cacheRefillFSM = new StateMachine {
        ...
        // uncached load
        val udBus = pipeline.service[UDBus].udBus
        val uncachedStoreHandshake = pipeline.service[UncachedAccess].uncachedStoreHandshake
