...
import zencove.util.MultiPortFIFOVec
import zencove.model._
import zencove.enum.RegWriteAddr
import scala.math
import zencove.util.MultiPortFIFOVecOutReg

class RenameByPRF(config: ZencoveConfig) extends Plugin[DecodePipeline] {
  ...
  override def build(pipeline: DecodePipeline): Unit = pipeline.RENAME plug new Area {
    ...
    for (i <- 0 until decPacket.size; valid = decPacket(i).valid; uop = decPacket(i).payload) {
      ...
      regReads(i)(0).req.valid := uop.useRs && valid
      regReads(i)(0).req.payload := fields.rs
      regReads(i)(1).req.valid := uop.useRt && valid
      regReads(i)(1).req.payload := fields.rt
