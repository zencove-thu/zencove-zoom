...
import zencove.blackbox.mem.{xpm_fifo_sync, xpm_fifo_sync_generic}
...
class AxiBuffer(config: Axi4Config, waitForB: Boolean) extends Component {
    val io = new Bundle {
        val in_axi = slave(Axi4(config))
        val out_axi = master(Axi4(config))
    }
    val bufferType = HardType(BufferEntry(config))
    ...
    val writeOutputFSM = new StateMachine {
        ...
        val waitW = new State
        val waitB = waitForB generate new State
        ...
        waitForB generate waitB.whenIsActive {
            writeIsRunning := True
            when(outb.valid) {
                goto(stateBoot)
            }
